...
class ICNetV5(SemanticSegmentationModel):
    ...
    def __small_branch_head(self, medium_branch_common_output: tf.Tensor, is_training: bool) -> tf.Tensor:
        ...
        bn_1 = tf.layers.batch_normalization(dil_1, training=is_training)
        pool_1 = self.__pyramid_pooling(bn_1, 128)
        conv_1_1 = tf.layers.conv2d(pool_1, 128, (3, 3), padding='SAME', activation='relu')
        bn_2 = tf.layers.batch_normalization(conv_1_1, training=is_training)
        conv_1_2 = tf.layers.conv2d(bn_2, 128, (3, 3), padding='SAME', activation='relu')
        bn_2 = tf.layers.batch_normalization(conv_1_2, training=is_training)
        add_1 = tf.math.add(bn_2, pool_1)
        ...
        conv_2_1 = tf.layers.conv2d(bn_4, 256, (3, 3), padding='SAME', activation='relu')
        bn_5 = tf.layers.batch_normalization(conv_2_1, training=is_training)
        conv_2_2 = tf.layers.conv2d(bn_5, 256, (3, 3), padding='SAME', activation='relu')
        bn_6 = tf.layers.batch_normalization(conv_2_2, training=is_training)
        add_2 = tf.math.add(pool_2, bn_6)
        ...
        conv_3_1 = tf.layers.conv2d(bn_8, 512, (3, 3), padding='SAME', activation='relu')
        dil_3_1_down = self.__filters_scaling(conv_3_1, 256)
        add_3 = tf.math.add(dil_3_1_down, add_2)

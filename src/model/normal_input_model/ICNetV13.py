...
class ICNetV13(SemanticSegmentationModel):
    ...
    def run(self, X: tf.Tensor, num_classes: int, is_training: bool = True, y: Optional[tf.Tensor] = None) -> Tuple[
        ...
        upsampled_2 = tf.layers.conv2d(upsampled_2, 128, (3, 3), padding='SAME', activation='relu')
        upsampled_2 = tf.layers.conv2d(upsampled_2, 16 * num_classes, (1, 1), padding='SAME', activation=None)
        upsample_4_size = upsampled_2.shape[1] * 4, upsampled_2.shape[2] * 4
        classifier = tf.reshape(upsampled_2, [-1, upsample_4_size[0], upsample_4_size[1], num_classes])
        cascade_loss_3 = None
        if is_training:
            y = tf.squeeze(y, axis=-1)
            cascade_loss_3 = self.__compute_loss(classifier, y)
        overall_loss = None
        if is_training:
    ...
    def __medium_branch_tail(self, medium_branch_common_output, is_training):
        ...
        bn_2 = tf.layers.batch_normalization(pool_1, training=is_training)
    ...
    def __small_branch_head(self, medium_branch_common_output: tf.Tensor, is_training: bool) -> tf.Tensor:
        ...
        conv_6 = self.__filters_scaling(pool_2, 64, name='small_branch_conv_6')
    ...
    def __pyramid_pooling(self, X: tf.Tensor, num_filters: int) -> tf.Tensor:
        ...
        pool_4 = tf.nn.pool(X, [8, 8], 'AVG', 'SAME', dilation_rate=[1, 1], strides=[2, 2])
        concat = tf.concat([pool_1, pool_2, pool_3, pool_4], axis=-1)

...
class ICNetV7(SemanticSegmentationModel):
    ...
    def __medium_branch_tail(self, medium_branch_common_output, is_training):
        dil_1 = self.__dilated_block(medium_branch_common_output, 128, is_training=is_training, residual=False)
        bn_1 = tf.layers.batch_normalization(dil_1, training=is_training)
        pool_1 = self.__pyramid_pooling(bn_1, 128, is_training=is_training)
        dil_2_1 = self.__dilated_block(pool_1, 128, is_training=is_training)
        bn_2 = tf.layers.batch_normalization(dil_2_1, training=is_training)
        dil_2_2 = self.__dilated_block(bn_2, 128, is_training=is_training)
    ...
    def __small_branch_head(self, medium_branch_common_output: tf.Tensor, is_training: bool) -> tf.Tensor:
        ...
        dil_1 = self.__dilated_block(input_scaled, 128, is_training=is_training, dim_red=False, residual=False)
        bn_1 = tf.layers.batch_normalization(dil_1, training=is_training)
        pool_1 = self.__pyramid_pooling(bn_1, 128, is_training=is_training)
        dil_2_1 = self.__dilated_block(pool_1, 128, is_training=is_training)
        bn_2 = tf.layers.batch_normalization(dil_2_1, training=is_training)
        ...
        pool_2 = self.__pyramid_pooling(bn_3, 256, is_training=is_training)
        dil_3_1 = self.__dilated_block(pool_2, 256, is_training=is_training)
        bn_4 = tf.layers.batch_normalization(dil_3_1, training=is_training)
        dil_3_2 = self.__dilated_block(bn_4, 256, is_training=is_training)
        add_2 = tf.math.add(dil_3_2, pool_2)

        bn_7 = tf.layers.batch_normalization(add_2, training=is_training)
        dil_3_5 = self.__dilated_block(bn_7, 512, is_training=is_training, residual=False)
        bn_8 = tf.layers.batch_normalization(dil_3_5, training=is_training)
        dil_3_6 = self.__dilated_block(bn_8, 512, is_training=is_training)
        dil_3_6_down = self.__filters_scaling(dil_3_6, 256)
        add_3 = tf.math.add(dil_3_6_down, add_2)

        return add_3

    def __dilated_block(self, X: tf.Tensor, output_filters: int, is_training: bool, dim_red: bool = True, residual: bool = True) -> tf.Tensor:
        if dim_red:
            feed_1 = self.__filters_scaling(X, 64)
            feed_1 = tf.layers.dropout(feed_1, rate=0.5, training=is_training)
            feed_2 = self.__filters_scaling(X, 64)
            feed_2 = tf.layers.dropout(feed_2, rate=0.5, training=is_training)
            feed_3 = self.__filters_scaling(X, 64)
            feed_3 = tf.layers.dropout(feed_3, rate=0.5, training=is_training)
            feed_4 = self.__filters_scaling(X, 64)
            feed_4 = tf.layers.dropout(feed_4, rate=0.5, training=is_training)
        else:
    ...
    def __pyramid_pooling(self, X: tf.Tensor, num_filters: int, is_training: bool) -> tf.Tensor:
        feed_1 = self.__filters_scaling(X, 64)
        feed_1 = tf.layers.dropout(feed_1, rate=0.5, training=is_training)

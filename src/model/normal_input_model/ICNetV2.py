...
class ICNetV2(SemanticSegmentationModel):
    ...
    def run(self, X: tf.Tensor, num_classes: int, is_training: bool = True, y: Optional[tf.Tensor] = None) -> Tuple[tf.Tensor, Optional[tf.Tensor]]:
        ...
        upsampled_2 = tf.layers.conv2d(upsampled_2, 16, (3, 3), padding='SAME', activation=None, dilation_rate=(2, 2))
        ...
        upsampled_4 = tf.layers.conv2d(upsampled_4, 16, (3, 3), padding='SAME', activation=None, dilation_rate=(2, 2))
        print('upsampled_4 {}'.format(upsampled_4.shape))
    ...
    def __medium_branch_tail(self, medium_branch_common_output, is_training):
        input_scaled = self.__filters_scaling(medium_branch_common_output, 32)
        ...
        add_1 = tf.math.add(dil_2_1, pool_1)
    ...
    def __small_branch_head(self, medium_branch_common_output: tf.Tensor, is_training: bool) -> tf.Tensor:
        ...
        add_2 = tf.math.add(dil_3_3, pool_2)
        return add_2
    ...
    def __cascade_fusion_block(self, smaller_input: tf.Tensor, bigger_input: tf.Tensor, is_training: bool, casc_op_number: int,
        ...
        upsampled = tf.layers.conv2d(upsampled, 32, (3, 3), padding='SAME', activation=None, dilation_rate=(2, 2))

...
class ICNetV17(SemanticSegmentationModel):
    ...
    def run(self, X: tf.Tensor, num_classes: int, is_training: bool = True, y: Optional[tf.Tensor] = None) -> Tuple[
        ...
        if is_training:
            ...
            overall_loss = tf.stack([0.16 * cascade_loss_1, 0.4 * cascade_loss_2, cascade_loss_3], axis=0)
    ...
    def __top_branch(self, input: tf.Tensor, is_training: bool) -> tf.Tensor:
        halved_conv = self.__downsample_conv_block(input, 32, (5, 5))
    ...
    def __medium_branch_head(self, half_input: tf.Tensor, is_training: bool) -> tf.Tensor:
        ...
        one_eiqght_conv_1 = self.__downsample_conv_block(bn_1, 64, (3, 3))
    ...
    def __dilated_block(self, X: tf.Tensor, output_filters: int, dim_red: bool = True,
        ...
        if dim_red:
            feed_1 = feed_2 = feed_3 = feed_4 = self.__filters_scaling(X, 128)
        else:
            feed_1 = feed_2 = feed_3 = feed_4 = X
        out_1 = tf.layers.conv2d(feed_1, output_filters / 2, (3, 3), padding='SAME', activation='relu')
        out_2 = tf.layers.conv2d(feed_2, output_filters / 2, (3, 3), padding='SAME', activation='relu',
                                 dilation_rate=(2, 2))
        out_3 = tf.layers.conv2d(feed_3, output_filters / 2, (3, 3), padding='SAME', activation='relu',
                                 dilation_rate=(4, 4))
        out_4 = tf.layers.conv2d(feed_4, output_filters / 2, (3, 3), padding='SAME', activation='relu',
                                 dilation_rate=(8, 8))
        out = tf.concat([out_1, out_2, out_3, out_4], axis=-1)
        out = tf.layers.conv2d(out, output_filters, (3, 3), padding='SAME', activation='relu')
        if residual:
    ...
    def __cascade_fusion_block(self, smaller_input: tf.Tensor, bigger_input: tf.Tensor, is_training: bool,
        ...
        upsampled = tf.layers.conv2d(upsampled, 128, (3, 3), padding='SAME', activation='relu')
        upsampled_bn = tf.layers.batch_normalization(upsampled, training=is_training)
        bigger_input = self.__filters_scaling(bigger_input, 128)
        print('{} {}'.format(upsampled_bn.shape, bigger_input.shape))
        out = tf.concat([upsampled_bn, bigger_input], axis=-1)
        out = tf.layers.conv2d(out, 128, (3, 3), padding='SAME', activation='relu')
        return out, cascade_loss

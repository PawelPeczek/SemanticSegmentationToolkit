...
class ICNetV6(SemanticSegmentationModel):
    ...
    def __dilated_block(self, X: tf.Tensor, output_filters: int, dim_red: bool = True, residual: bool = True) -> tf.Tensor:
        if dim_red:
            ...
            feed_1 = tf.keras.layers.SpatialDropout2D(rate=0.4)(feed_1)
            feed_2 = self.__filters_scaling(X, 64)
            feed_2 = tf.keras.layers.SpatialDropout2D(rate=0.4)(feed_2)
            feed_3 = self.__filters_scaling(X, 64)
            feed_3 = tf.keras.layers.SpatialDropout2D(rate=0.4)(feed_3)
            feed_4 = self.__filters_scaling(X, 64)
            feed_4 = tf.keras.layers.SpatialDropout2D(rate=0.4)(feed_4)
        else:

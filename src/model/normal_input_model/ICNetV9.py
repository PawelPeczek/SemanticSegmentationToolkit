...
class ICNetV9(SemanticSegmentationModel):
    ...
    def run(self, X: tf.Tensor, num_classes: int, is_training: bool = True, y: Optional[tf.Tensor] = None) -> Tuple[tf.Tensor, Optional[tf.Tensor]]:
        ...
        if is_training:
            ...
            overall_loss = tf.stack([0.064 * cascade_loss_1, 0.16 * cascade_loss_2, 0.4 * cascade_loss_3, main_loss], axis=0)
    ...
    def __small_branch_head(self, medium_branch_common_output: tf.Tensor, is_training: bool) -> tf.Tensor:
        ...
        conv_11 = self.__filters_scaling(bn_8, 64, name='small_branch_conv_11')
        conv_12 = tf.layers.conv2d(conv_11, 512, (3, 3), padding='SAME', activation='relu', name='small_branch_conv_12')
        conv_13 = self.__filters_scaling(conv_12, 64, name='small_branch_conv_13')
        conv_14 = tf.layers.conv2d(conv_13, 512, (3, 3), padding='SAME', activation='relu', name='small_branch_conv_14')
        conv_15 = self.__filters_scaling(conv_14, 64, name='small_branch_conv_15')
        conv_16 = tf.layers.conv2d(conv_15, 768, (3, 3), padding='SAME', activation='relu', name='small_branch_conv_16')
        conv_17 = self.__filters_scaling(conv_16, 128, name='small_branch_conv_17')
        conv_18 = tf.layers.conv2d(conv_17, 1024, (3, 3), padding='SAME', activation='relu', name='small_branch_conv_18')
        conv_19 = self.__filters_scaling(conv_18, 256, name='small_branch_conv_19')
        add_3 = tf.math.add(conv_19, add_2)

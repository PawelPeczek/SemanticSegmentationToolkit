...
class ICNetV10(SemanticSegmentationModel):
    ...
    def run(self, X: tf.Tensor, num_classes: int, is_training: bool = True, y: Optional[tf.Tensor] = None) -> Tuple[
        ...
        if is_training:
            ...
            main_loss = self.__compute_loss(classifier, y)
        overall_loss = None
        if is_training:
    ...
    def __small_branch_head(self, medium_branch_common_output: tf.Tensor, is_training: bool) -> tf.Tensor:
        ...
        conv_18 = tf.layers.conv2d(conv_17, 1024, (3, 3), padding='SAME', activation='relu',

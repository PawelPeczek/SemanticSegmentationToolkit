...
class ThinModelV3(SemanticSegmentationModel):
    ...
    def run(self, X: tf.Tensor, num_classes: int, is_training: bool = True, y: Optional[tf.Tensor] = None) -> Union[tf.Tensor, Optional[tf.Tensor]]:
        ...
        deconv_2 = self.__deconv_block(dec_res_1_conv, 32)
        ...
        model_out = self.__filters_scaling(dec_res_3_conv, num_classes, activation=None)
        loss = None
        if is_training:
    ...
    def __dilated_block(self, X: tf.Tensor, output_slices: int) -> tf.Tensor:
        ...
        out_1 = tf.layers.conv2d(X, slice_depth, (5, 5), padding='SAME', activation='relu')
    ...
    def __filters_scaling(self, X: tf.Tensor, num_filters: int, activation: Union[str, None] = 'relu') -> tf.Tensor:
        return tf.layers.conv2d(X, num_filters, (1, 1), padding='SAME', activation=activation)

    def __pyramid_pooling(self, X: tf.Tensor, output_slices: int) -> tf.Tensor:
        slice_depth = int(output_slices / 4)

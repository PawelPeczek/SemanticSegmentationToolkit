...
class ThinModelV1(SemanticSegmentationModel):
    ...
    def run(self, X, num_classes, is_training=True):
        ...
        bn_1 = tf.layers.batch_normalization(dil_1, is_training=is_training)
        pool_1 = self.__pyramid_pooling(bn_1)
        dil_2_1 = self.__dilated_block(pool_1)
        bn_2 = tf.layers.batch_normalization(dil_2_1, is_training=is_training)
        dil_2_2 = self.__dilated_block(bn_2)
        res_2 = tf.math.add(dil_2_2, pool_1)
        
        bn_3 = tf.layers.batch_normalization(res_2, is_training=is_training)
        pool_2 = self.__pyramid_pooling(bn_3)
        dil_3_1 = self.__dilated_block(pool_2)
        bn_4 = tf.layers.batch_normalization(dil_3_1, is_training=is_training)
        dil_3_2 = self.__dilated_block(bn_4)
        bn_5 = tf.layers.batch_normalization(dil_3_2, is_training=is_training)
        dil_3_3 = self.__dilated_block(bn_5)
        res_3 = tf.math.add(dil_3_3, pool_2)
        
        bn_6 = tf.layers.batch_normalization(res_3, is_training=is_training)
        pool_3 = self.__pyramid_pooling(bn_6)
        dil_4_1 = self.__dilated_block(pool_3)
        bn_7 = tf.layers.batch_normalization(dil_4_1, is_training=is_training)
        dil_4_2 = self.__dilated_block(bn_7)
        bn_8 = tf.layers.batch_normalization(dil_4_2, is_training=is_training)
        dil_4_3 = self.__dilated_block(bn_8)
        res_4 = tf.math.add(dil_4_3, pool_3)
        
        #decoder
        deconv_1 = self.__deconv_block(res_4)
        bn_9 = tf.layers.batch_normalization(deconv_1, is_training=is_training)
        dec_res_1 = tf.math.add(bn_9, pool_2)
        deconv_2 = self.__deconv_block(dec_res_1)
        bn_10 = tf.layers.batch_normalization(deconv_2, is_training=is_training)
        dec_res_2 = tf.math.add(bn_10, pool_1)
        deconv_3 = self.__deconv_block(dec_res_2)
        bn_11 = tf.layers.batch_normalization(deconv_3, is_training=is_training)
    ...
    def __dilated_block(self, X, dim_red=True, residual=True):
        ...
        out = tf.concat([out_1, out_2, out_3, out_4], 3)
        if residual:
    ...
    def __pyramid_pooling(self, X):
        ...
        out = tf.concat([pool_1, pool_2, pool_3, pool_4], 3)
